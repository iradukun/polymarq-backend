...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(self, email, username=None, password=None, **extra_fields) -> UserType:
        if not email:
    ...
    def create_user_with_phone(self, phone_number, username, password=None, **extra_fields) -> UserType:
        if not phone_number:
            raise ValueError("The Phone Number field must be set")

        try:
            username_value = username if username is not None or "" else phone_number
            user = self.model(phone_number=phone_number, username=username_value, **extra_fields)
            user.set_password(password)
            user.save(using=self._db)
            return user
