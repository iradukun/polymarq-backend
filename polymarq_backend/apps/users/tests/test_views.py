...
from polymarq_backend.apps.users.forms import UserAdminChangeForm
from polymarq_backend.apps.users.models import Client, Technician, TechnicianType, User
from polymarq_backend.apps.users.tests.factories import BaseTestCase, UserFactory
from polymarq_backend.apps.users.views import UserRedirectView, UserUpdateView, user_detail_view
...
class TestTechnicianView(BaseTestCase):
    ...
    def test_register_technician(self):
        ...
        response = self.client.post(url, self.technician_data, content_type="application/json")
        response_json = response.json()

        self.assertEqual(response.status_code, 201)
        self.assertEqual(response_json.get("message"), "Technician registered successfully")
    ...
    def test_get_technicians_list(self):
        ...
        self.assertEqual(response_json["result"]["data"][0]["uuid"], str(self.user.uuid))
        self.assertEqual(response_json["result"]["data"][0]["professionalSummary"], self.professional_summary)

    def test_get_single_technician(self):
        url = reverse("api:authorized-technician-user-profile")
        response = self.client.get(url, headers=self.headers)
        response_json = response.json()
        ...
        self.assertEqual(response_json["result"]["professionalSummary"], self.professional_summary)
    ...
    def test_update_single_technician(self):
        ...
        response = self.client.patch(
            url, data=dict(work_address=work_address), headers=self.headers, content_type="application/json"
        )
        response_json = response.json()
...
class TechnicianTypeView(BaseTestCase):
    ...
    def test_create_technician_type(self):
        ...
        response = self.client.post(url, data=dict(title=title), headers=self.headers, content_type="application/json")
        response_json = response.json()
    ...
    def test_update_single_technician_type(self):
        ...
        response = self.client.patch(
            url, data=dict(title=title), headers=self.headers, content_type="application/json"
        )
        response_json = response.json()
...
class TestClientView(BaseTestCase):
    ...
    def test_register_client(self):
        ...
        response = self.client.post(url, self.client_data, content_type="application/json")
        response_json = response.json()
    ...
    def test_get_single_client(self):
        url = reverse("api:authorized-client-user-profile")
        response = self.client.get(url, headers=self.headers)
        response_json = response.json()
    ...
    def test_update_single_client(self):
        ...
        response = self.client.patch(
            url, data=dict(address=address), headers=self.headers, content_type="application/json"
        )
        response_json = response.json()
