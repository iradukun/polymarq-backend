...
from polymarq_backend.apps.users.api.serializers import ErrorResponseChildSerializer
...
def handle_and_validate_errors_list(errors: list[ErrorDict] | None) -> list[ErrorDict] | None:
    if errors is None:
        return None
    ...
    for error in errors:
        ...
        if not serializer.is_valid():
            raise ValidationError(f"Invalid error structure: {json.dumps(error, indent=4)}")
    return errors
